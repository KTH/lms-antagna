...
/** Get the name of the UG group name of antagna students  */
function getAntagnaGroupName (courseCode, term, round) {
  ...
  return `ladok2.kurser.${prefix}.${suffix}.antagna_${term}.${round}`
}

async function getAntagna (courseCode, term, round) {
  const groupName = getAntagnaGroupName(courseCode, term, round)
  ...
  const searchKthIds = peopleDNs
    .filter(dn => dn)
    .map(dn => ldapSearch({ base: dn, scope: 'base', attributes: ['ugKthId'] }))
