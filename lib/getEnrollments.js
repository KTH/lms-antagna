...
// Written as constant to avoid mistypes
const ANTAGNA = 'Admitted not registered'
...
module.exports = {
  ...
  async toRemoveAntagna (period) {
    ...
    for (const round of courseRounds) {
      ...
      try {
        ...
        log.debug(`CANVAS: ${progress}. Antagna found in [${round.sis_id.padStart(12)}] : ${roundEnrollments.length}`)
        ...
        if (err.name === 'HTTPError' && err.statusCode === 404) {
          log.warn(err, `CANVAS: ${progress}. Error. [${round.sis_id.padStart(12)}] not found`)
        } else {
          err.message = `CANVAS: Error when getting enrollments in [${round.sis_id}]: ` + err.message
          throw err
        }
      }
    }

    log.info(`CANVAS: Total antagna students in ${period}: ${allEnrollments.length}`)
  ...
  async toAddAntagna (period) {
    ...
    for (const round of courseRounds) {
      ...
      try {
        const kthIds = await ug.getAntagna(round.course_code, period.koppsTerm(), round.round_id)
        log.debug(`UG: ${progress}. Antagna found for [${round.sis_id.padStart(12)}]: ${kthIds.length}`)
        ...
        err.message = `UG: Error when getting antagna for [${round.sis_id}]: ` + err.message
        throw err
      }
    }
